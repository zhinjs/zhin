...
- [Zhin Plugin-first Target Architecture (SSOT)](./docs/target-architecture.md) - 定义 Plugin 树、Feature/Resource 边界、HMR、消息、Agent 和 Console 的规范性目标。
- [Plugin Runtime](./packages/im/plugin-runtime/CONTEXT.md) - 负责 Plugin tree、generation、Root lifecycle，以及与 Process Host 的 ownership 分工。
...
- [HTTP Host](./packages/host/http/) - Node HTTP/WebSocket、Bearer、CORS、OpenAPI 与 Console transport。
- [Protocol Hosts](./packages/host/mcp/) / [A2A](./packages/host/a2a/) - 建立在最小 HTTP route contract 之上的 MCP 与 Agent-to-Agent 协议。
...
- **全局装配关系**：Plugin 是唯一基础单元；Resource 与 Feature contribution 统一附着在 PluginScope，Runtime 只读 snapshot。详细关系以 [目标架构](./docs/target-architecture.md) 为准。
- **Process Host -> Plugin Runtime**：Process Host 独占操作系统信号和进程退出；Plugin Runtime 只执行可嵌入、幂等的 Runtime Stop。
