...
export function registerLogsRoutes(
  ...
  registerFetchRoute(table, "GET", `${base}/logs`, async (ctx: RouterContext) => {
  ...
  registerFetchRoute(table, "DELETE", `${base}/logs`, async (ctx: RouterContext) => {
    const LogModel = getModel();
    if (!LogModel) {
  ...
  registerFetchRoute(table, "GET", `${base}/logs/stats`, async (ctx: RouterContext) => {
    const LogModel = getModel();
    if (!LogModel) {
      ctx.body = {
        success: true,
  ...
  registerFetchRoute(table, "POST", `${base}/logs/cleanup`, async (ctx: RouterContext) => {
    const LogModel = getModel();
    if (!LogModel) {
