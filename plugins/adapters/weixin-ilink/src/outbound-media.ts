...
export function segmentMediaRef(seg: MessageSegment): string | undefined {
  const data = seg.data as Record<string, unknown>;
  const candidates = [data.file, data.path, data.url].filter((v): v is string => typeof v === "string" && Boolean(v));
  return candidates[0];
}
...
function resolveExt(seg: MessageSegment, mime?: string): string {
  const data = seg.data as Record<string, unknown>;
  ...
  const mimeHint = mime ?? (typeof data.mime === "string" ? data.mime : typeof data.mimeType === "string" ? data.mimeType : undefined);
  if (mimeHint) {
...
function withLocalFile(seg: MessageSegment, filePath: string): MessageSegment {
  const data = { ...seg.data, file: filePath } as Record<string, unknown>;
  delete data.base64;
  return { type: seg.type, data } as MessageSegment;
}

function writeOutboundBuffer(buffer: Buffer, outboundDir: string, seg: MessageSegment): string {
...
function asSegments(content: SendContent): MessageSegment[] {
  ...
  return content.flatMap((item) => {
    if (typeof item === "string") return [{ type: "text", data: { text: item } } as MessageSegment];
    return typeof item === "object" && item !== null && "type" in item ? [item as MessageSegment] : [];
  });
}
...
export async function materializeOutboundMedia(
  ...
  if (typeof content === "string") {
    return out[0]?.type === "text" ? (out[0].data as { text: string }).text : content;
  }
  if (!Array.isArray(content)) {
