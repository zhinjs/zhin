...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'onebot12-cmd-'));
...
afterEach(() => {
  ...
  delete process.env.ONEBOT12_BOT1_ACCESS_TOKEN;
...
describe('onebot12.endpoint command definitions', () => {
  ...
  it('add 走 kv 参数：凭据写 .env，yaml 存 ${REF}，连接字段内联', () => {
    ...
    expect(fs.readFileSync(path.join(root, '.env'), 'utf-8')).toContain('ONEBOT12_BOT1_ACCESS_TOKEN=sec-1');
  ...
  it('配置 master 后非 master 拒绝 add/remove', () => {
    ...
    expect(addCommand.execute(denied)).toBe('仅 master 可执行 OneBot 12 endpoint 管理命令');
    expect(removeCommand.execute(denied)).toBe('仅 master 可执行 OneBot 12 endpoint 管理命令');
  });
});
