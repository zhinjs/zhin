 ...
 * OneBot12 适配器集成测试
 *
 * 策略：Mock 掉 WebSocket 传输层和 callAction，测试 Endpoint 接口合规性、
...
import { OneBot12WsClient } from '../src/endpoint-ws.js';
...
class MockOneBot12Endpoint extends OneBot12WsClient {
...
class MockOneBot12Adapter extends OneBot12Adapter {
  createEndpoint(config: OneBot12WsConfig): MockOneBot12Endpoint {
    return new MockOneBot12Endpoint(this, {
      context: 'onebot12',
...
createAdapterTestSuite<MockOneBot12Adapter, OneBot12Event>({
  ...
  createAdapter: (plugin) => {
    ...
    (adapter as any).config = [{ name: 'test-endpoint', context: 'onebot12', url: 'ws://mock:8080', connection: 'ws' }];
    return adapter;
  },
...
describe('OneBot12 适配器特定测试', () => {
  ...
  let endpoint: MockOneBot12Endpoint;
  ...
  beforeEach(async () => {
    ...
    (adapter as any).config = [{ name: 'test-endpoint', context: 'onebot12', url: 'ws://mock:8080', connection: 'ws' }];
    await adapter.start();
    endpoint = adapter.endpoints.get('test-endpoint') as MockOneBot12Endpoint;
  });
