...
import listCommand from '../commands/wechat-mp/endpoint/list/index.js';
import addCommand from '../commands/wechat-mp/endpoint/add/[id]/index.js';
import removeCommand from '../commands/wechat-mp/endpoint/remove/[id]/index.js';
...
describe('wechat-mp endpoint command definitions', async () => {
  ...
  it('list 显示运行中 + 配置中的 endpoints', async () => {
    ...
    await addCommand.execute(fakeContext({ params: { id: 'conf-bot' }, args: ['appId=wx-1', 'appSecret=sec-1', 'token=tok-1'] }));
  ...
  it('remove 从配置移除并提示重启', async () => {
    await addCommand.execute(fakeContext({ params: { id: 'bot1' }, args: ['appId=wx-1', 'appSecret=sec-1', 'token=tok-1'] }));
  ...
  it('配置 master 后非 master 拒绝 add/remove', async () => {
    ...
    expect(await addCommand.execute(denied)).toBe('仅 master 可执行 WeChat MP endpoint 管理命令');
    expect(await removeCommand.execute(denied)).toBe('仅 master 可执行 WeChat MP endpoint 管理命令');
  });
});
