...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'telegram-cmd-'));
...
afterEach(() => {
  ...
  delete process.env.TELEGRAM_BOT1_TOKEN;
...
describe('telegram.endpoint command definitions', () => {
  ...
  it('add 走 kv 参数：token 写 .env，yaml 存 ${REF}', () => {
    ...
    expect(fs.readFileSync(path.join(root, '.env'), 'utf-8')).toContain('TELEGRAM_BOT1_TOKEN=123:abc');
  ...
  it('add 缺少必填 token 时报错', () => {
    expect(addCommand.execute(fakeContext({ params: { name: 'bot1' } }))).toContain('缺少必填字段：token');
  });

  it('list 显示运行中 + 配置中的 endpoints', () => {
    const context = fakeContext();
    (context as { state: ReturnType<typeof createEndpointRuntimeState> }).state
      .endpoints.set('bot1', { name: 'bot1', mode: 'polling' });
    addCommand.execute(fakeContext({ params: { name: 'conf-bot' }, args: ['token=t'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { name: 'bot1' }, args: ['token=t'] }));
  ...
  it('配置 master 后非 master 拒绝 add/remove', () => {
    ...
    expect(addCommand.execute(denied)).toBe('仅 master 可执行 Telegram endpoint 管理命令');
    expect(removeCommand.execute(denied)).toBe('仅 master 可执行 Telegram endpoint 管理命令');
  });
});
