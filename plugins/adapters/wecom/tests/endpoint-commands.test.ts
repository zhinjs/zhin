...
describe('wecom.endpoint command definitions', () => {
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    addCommand.execute(fakeContext({ params: { id: 'conf-bot' }, args: ['corpId=ww-1', 'agentSecret=sec-1', 'token=tok-1', 'encodingAESKey=aes-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { id: 'bot1' }, args: ['corpId=ww-1', 'agentSecret=sec-1', 'token=tok-1', 'encodingAESKey=aes-1'] }));
