...
describe('line.endpoint command definitions', () => {
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    addCommand.execute(fakeContext({ params: { id: 'conf-bot' }, args: ['channelSecret=cs-1', 'channelAccessToken=cat-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { id: 'bot1' }, args: ['channelSecret=cs-1', 'channelAccessToken=cat-1'] }));
