...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'line-cmd-'));
...
afterEach(() => {
  ...
  delete process.env.LINE_BOT1_CHANNEL_SECRET;
  delete process.env.LINE_BOT1_CHANNEL_ACCESS_TOKEN;
...
describe('line.endpoint command definitions', () => {
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    addCommand.execute(fakeContext({ params: { name: 'conf-bot' }, args: ['channelSecret=cs-1', 'channelAccessToken=cat-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { name: 'bot1' }, args: ['channelSecret=cs-1', 'channelAccessToken=cat-1'] }));
  ...
  it('配置 master 后非 master 拒绝 add/remove', () => {
    ...
    expect(addCommand.execute(denied)).toBe('仅 master 可执行 LINE endpoint 管理命令');
    expect(removeCommand.execute(denied)).toBe('仅 master 可执行 LINE endpoint 管理命令');
  });
});
