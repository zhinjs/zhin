...
import { SatoriWsClient } from '../src/endpoint-ws.js';
...
class MockSatoriEndpoint extends SatoriWsClient {
  callApiMock = vi.fn();
...
class MockSatoriAdapter extends SatoriAdapter {
  createEndpoint(config: SatoriWsConfig): MockSatoriEndpoint {
    return new MockSatoriEndpoint(this, {
      context: 'satori',
...
createAdapterTestSuite<MockSatoriAdapter, any>({
  ...
  createAdapter: (plugin) => {
    ...
    (adapter as any).config = [{ name: 'test-endpoint', context: 'satori', baseUrl: 'ws://mock:5500', connection: 'ws' }];
    return adapter;
  },
...
describe('Satori 适配器特定测试', () => {
  ...
  let endpoint: MockSatoriEndpoint;
  ...
  beforeEach(async () => {
    ...
    (adapter as any).config = [{ name: 'test-endpoint', context: 'satori', baseUrl: 'ws://mock:5500', connection: 'ws' }];
    await adapter.start();
    endpoint = adapter.endpoints.get('test-endpoint') as MockSatoriEndpoint;
  });
