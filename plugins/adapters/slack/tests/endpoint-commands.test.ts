...
describe('slack endpoint command definitions', () => {
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    (context as { state: ReturnType<typeof createEndpointRuntimeState> }).state
      .endpoints.set('bot1', { name: 'bot1', mode: 'socket' });
    addCommand.execute(fakeContext({ params: { name: 'conf-bot' }, args: ['token=xoxb-1', 'appToken=xapp-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { name: 'bot1' }, args: ['token=xoxb-1', 'appToken=xapp-1'] }));
