...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'milky-cmd-'));
...
afterEach(() => {
  ...
  delete process.env.MILKY_BOT1_ACCESS_TOKEN;
...
describe('milky.endpoint command definitions', () => {
  ...
  it('add 走 kv 参数：凭据写 .env，yaml 存 ${REF}，连接字段内联', () => {
    ...
    expect(fs.readFileSync(path.join(root, '.env'), 'utf-8')).toContain('MILKY_BOT1_ACCESS_TOKEN=sec-1');
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    addCommand.execute(fakeContext({ params: { name: 'conf-bot' }, args: ['baseUrl=http://127.0.0.1:8080', 'access_token=sec-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { name: 'bot1' }, args: ['baseUrl=http://127.0.0.1:8080', 'access_token=sec-1'] }));
  ...
  it('配置 master 后非 master 拒绝 add/remove', () => {
    ...
    expect(addCommand.execute(denied)).toBe('仅 master 可执行 Milky endpoint 管理命令');
    expect(removeCommand.execute(denied)).toBe('仅 master 可执行 Milky endpoint 管理命令');
  });
});
