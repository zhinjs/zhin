...
function fakeContext(overrides: Record<string, unknown> = {}) {
  ...
  return {
    state,
    use: (token: unknown) => {
      if (token === milkyRuntimeStateToken) return state;
...
describe('milky endpoint command definitions', async () => {
  ...
  it('add 走 kv 参数：凭据写 .env，yaml 存 ${REF}，连接字段内联', async () => {
    ...
    expect(store.environmentText()).toContain('MILKY_BOT1_ACCESS_TOKEN=sec-1');
    const config = store.configurationText('milky');
    expect(config).toContain('baseUrl: http://127.0.0.1:8080');
    expect(config).toContain('${MILKY_BOT1_ACCESS_TOKEN}');
  });
  ...
  it('list 显示运行中 + 配置中的 endpoints', async () => {
    ...
    await addCommand.execute(fakeContext({ params: { id: 'conf-bot' }, args: ['baseUrl=http://127.0.0.1:8080', 'access_token=sec-1'] }));
  ...
  it('remove 从配置移除并提示重启', async () => {
    await addCommand.execute(fakeContext({ params: { id: 'bot1' }, args: ['baseUrl=http://127.0.0.1:8080', 'access_token=sec-1'] }));
    ...
    expect(store.configurationText('milky')).not.toContain('bot1');
  });
  ...
  it('配置 master 后非 master 拒绝 add/remove', async () => {
    ...
    expect(await addCommand.execute(denied)).toBe('仅 master 可执行 Milky endpoint 管理命令');
    expect(await removeCommand.execute(denied)).toBe('仅 master 可执行 Milky endpoint 管理命令');
  });
});
