...
describe('milky.endpoint command definitions', () => {
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    addCommand.execute(fakeContext({ params: { id: 'conf-bot' }, args: ['baseUrl=http://127.0.0.1:8080', 'access_token=sec-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { id: 'bot1' }, args: ['baseUrl=http://127.0.0.1:8080', 'access_token=sec-1'] }));
