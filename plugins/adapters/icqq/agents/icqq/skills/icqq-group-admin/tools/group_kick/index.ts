...
export default defineAgentTool<{ group_id: number; user_id: number }>({
  ...
  async execute({ group_id, user_id }, context) {
    ...
    if(!selfInfo.is_admin && !selfInfo.is_owner) return { success: false, message: '机器人不是群主或管理员，无法踢出成员' };
    if(selfInfo.user_id === user_id) return { success: false, message: '机器人不能踢出自己' };
