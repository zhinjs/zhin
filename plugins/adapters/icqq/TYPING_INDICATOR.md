...
1. **连接时初始化**：Endpoint 连接成功后，根据配置自动初始化 Typing Indicator
...
5. **配置优先级**：Endpoint 级别的配置会覆盖适配器级别的默认配置
...
```typescript
import { IcqqEndpoint } from '@zhin.js/adapter-icqq';

// 检查 Endpoint 是否启用了 Typing Indicator
function hasTypingIndicator(endpoint: IcqqEndpoint): boolean {
  return !!endpoint.$typingIndicator;
}

// 手动开始提示
async function startTyping(endpoint: IcqqEndpoint, messageId: string, sessionId: string) {
  if (endpoint.$typingIndicator) {
    return await endpoint.$typingIndicator.start({
      messageId,
      sessionId,
...
// 手动停止提示
async function stopTyping(bot: IcqqEndpoint, sessionId: string) {
  if (endpoint.$typingIndicator) {
    await endpoint.$typingIndicator.stop({ sessionId });
  }
}
```
