/**
 * OneBot11 WebSocket 客户端连接 Endpoint
 */
...
export class OneBot11WsClient extends EventEmitter implements Endpoint<OneBot11WsClientConfig, OneBot11Message> {
  $connected: boolean;
    ...
    const message = Message.from(onebotMsg, {
      ...
      $sender: (() => {
        const sender: { id: string; name: string; role?: string; permissions?: string[] } = {
          ...
          name: onebotMsg.sender?.nickname || onebotMsg.sender?.card || onebotMsg.user_id.toString(),
        };
        applyQqSenderRoleToMessageSender(sender, onebotMsg.sender?.role);
        return sender;
      })(),
      $channel: {
  ...
  async kickMember(groupId: number, userId: number, rejectAddRequest: boolean = false): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 踢出成员失败:`, error);
      throw error;
    }
  }
  ...
  async muteMember(groupId: number, userId: number, duration: number = 600): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 禁言操作失败:`, error);
      throw error;
    }
  }
  ...
  async muteAll(groupId: number, enable: boolean = true): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 全员禁言操作失败:`, error);
      throw error;
    }
  }
  ...
  async setAdmin(groupId: number, userId: number, enable: boolean = true): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 设置管理员失败:`, error);
      throw error;
    }
  }
  ...
  async setCard(groupId: number, userId: number, card: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 设置群名片失败:`, error);
      throw error;
    }
  }
  ...
  async setTitle(groupId: number, userId: number, title: string, duration: number = -1): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 设置头衔失败:`, error);
      throw error;
    }
  }
  ...
  async setGroupName(groupId: number, name: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 设置群名失败:`, error);
      throw error;
    }
  }
  ...
  async getMemberList(groupId: number): Promise<any[]> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 获取群成员列表失败:`, error);
      throw error;
    }
  }
  ...
  async getGroupInfo(groupId: number): Promise<any> {
    try {
      ...
      this.logger.error(`OneBot11 Endpoint ${this.$id} 获取群信息失败:`, error);
      throw error;
    }
  }
