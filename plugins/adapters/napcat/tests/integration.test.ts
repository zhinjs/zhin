 ...
 * NapCat 适配器集成测试
 *
 * Mock 掉 WebSocket 传输层，测试 Endpoint 接口合规性、消息格式化、
 * 发送/接收链路、入站治理。
 */
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { Plugin } from 'zhin.js';
import { createAdapterTestSuite } from '../../../../packages/im/core/tests/adapter-harness.js';
import { NapCatAdapter, type NapCatEndpoint } from '../src/adapter';
import { NapCatWsClient } from '../src/endpoint-ws-client';
import type { NapCatWsClientConfig, NapCatMessageEvent } from '../src/types';

const FIXED_TS = 1700000000000;

class MockNapCatEndpoint extends NapCatWsClient {
...
class MockNapCatAdapter extends NapCatAdapter {
  createEndpoint(config: any): NapCatEndpoint {
    return new MockNapCatEndpoint(this, {
      context: 'napcat',
      connection: 'ws',
      url: 'ws://mock:3001',
      name: config.name || 'test-endpoint',
      ...config,
    }) as unknown as NapCatEndpoint;
  }
}
...
createAdapterTestSuite<MockNapCatAdapter, NapCatMessageEvent>({
  ...
  createAdapter: (plugin) => {
    ...
    (adapter as any).config = [{ name: 'test-endpoint', connection: 'ws', url: 'ws://mock:3001', context: 'napcat' }];
    return adapter;
  },
...
describe('NapCat 适配器特定测试', () => {
  ...
  let endpoint: MockNapCatEndpoint;
  ...
  beforeEach(async () => {
    ...
    (adapter as any).config = [{ name: 'test-endpoint', connection: 'ws', url: 'ws://mock:3001', context: 'napcat' }];
    await adapter.start();
    endpoint = adapter.endpoints.get('test-endpoint') as MockNapCatEndpoint;
  });
  ...
  describe('Endpoint 接口合规性', () => {
    ...
    it('应实现 $addReaction 和 $removeReaction', () => {
      expect(typeof endpoint.$addReaction).toBe('function');
      expect(typeof endpoint.$removeReaction).toBe('function');
    });
  });
  ...
  describe('$addReaction', () => {
    it('应调用 set_msg_emoji_like', async () => {
      endpoint.callApiMock.mockResolvedValueOnce({});
      const result = await endpoint.$addReaction('12345', '128516');
      expect(endpoint.callApiMock).toHaveBeenCalledWith('set_msg_emoji_like', { message_id: 12345, emoji_id: '128516' });
  ...
  describe('入站消息治理', () => {
    it('自发消息 (message_sent) 不应触发 message.receive', async () => {
      ...
      (endpoint as any).dispatchEvent(raw);
  ...
  describe('$reply 路由', () => {
    ...
    it('字符串 content + quote 不应抛错（AI 未就绪等场景）', async () => {
      ...
      const msg = endpoint.$formatMessage(createNapCatRawEvent());
