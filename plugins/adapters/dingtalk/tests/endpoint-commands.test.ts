...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'dingtalk-cmd-'));
...
afterEach(() => {
  ...
  delete process.env.DINGTALK_BOT1_APP_KEY;
  delete process.env.DINGTALK_BOT1_APP_SECRET;
  delete process.env.DINGTALK_BOT1_ROBOT_CODE;
...
function fakeContext(overrides: Record<string, unknown> = {}) {
  ...
  return {
    state,
    use: (token: unknown) => {
      if (token === dingtalkRuntimeStateToken) return state;
...
describe('dingtalk.endpoint command definitions', () => {
  ...
  it('add 走 kv 参数：凭据写 .env，yaml 存 ${REF}', () => {
    const text = addCommand.execute(fakeContext({
      params: { name: 'bot1' },
      args: ['appKey=ak-1', 'appSecret=as-1', 'robotCode=rc-1'],
    })) as string;

    expect(text).toContain('✅');
    const envContent = fs.readFileSync(path.join(root, '.env'), 'utf-8');
    expect(envContent).toContain('DINGTALK_BOT1_APP_KEY=ak-1');
    expect(envContent).toContain('DINGTALK_BOT1_APP_SECRET=as-1');
    expect(envContent).toContain('DINGTALK_BOT1_ROBOT_CODE=rc-1');
    const config = fs.readFileSync(path.join(root, 'zhin.config.yml'), 'utf-8');
    expect(config).toContain('name: bot1');
    expect(config).toContain('${DINGTALK_BOT1_APP_KEY}');
  });

  it('add 缺少必填字段时报错', () => {
    expect(addCommand.execute(fakeContext({ params: { name: 'bot1' } })))
      .toContain('缺少必填字段：appKey、appSecret、robotCode');
  });
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    addCommand.execute(fakeContext({ params: { name: 'conf-bot' }, args: ['appKey=ak-1', 'appSecret=as-1', 'robotCode=rc-1'] }));
  ...
  it('remove 从配置移除并提示重启', () => {
    addCommand.execute(fakeContext({ params: { name: 'bot1' }, args: ['appKey=ak-1', 'appSecret=as-1', 'robotCode=rc-1'] }));
  ...
  it('配置 master 后非 master 拒绝 add/remove', () => {
    ...
    expect(addCommand.execute(denied)).toBe('仅 master 可执行 DingTalk endpoint 管理命令');
    expect(removeCommand.execute(denied)).toBe('仅 master 可执行 DingTalk endpoint 管理命令');
  });
});
