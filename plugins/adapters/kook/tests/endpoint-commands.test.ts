...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'kook-cmd-'));
...
afterEach(() => {
  ...
  delete process.env.KOOK_BOT1_TOKEN;
...
describe('kook.endpoint command definitions', () => {
  ...
  it('add 走 kv 参数：token 写 .env，yaml 存 ${REF}', () => {
    ...
    expect(fs.readFileSync(path.join(root, '.env'), 'utf-8')).toContain('KOOK_BOT1_TOKEN=tok-1');
  ...
  it('list 显示运行中 + 配置中的 endpoints', () => {
    ...
    (context as { state: ReturnType<typeof createEndpointRuntimeState> }).state
      .endpoints.set('bot1', { name: 'bot1', mode: 'websocket' });
