...
- **唯一入口：Plugin Runtime**：`plugin.ts` default-export `definePlugin()`，用 `zhin runtime start` 启动；能力用约定目录（`defineCommand` / `defineMiddleware` / `defineAgentTool` 等）。`zhin.js/node` 与 `bootstrapNode` **已删除且不再导出**。
