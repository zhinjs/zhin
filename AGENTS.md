...
- **唯一入口：Plugin Runtime**：`plugin.ts` default-export `definePlugin()`，用 `zhin runtime start` 启动；单文件能力入口使用 `$name.ts`。目录能力统一使用命名目录：`tools/<name>/index.ts`、`skills/<name>/SKILL.md`、`hooks/<name>/index.ts`、`agents/<name>/agent.json`。Agent/Skill 私有能力放在所属目录内并沿用同一结构。主 Agent 使用插件根 `AGENTS.md`；子 Agent 必须同时包含 `system.md`、`boundaries.md`、`conventions.md`，后者只能延伸根规则。`zhin.js/node` 与 `bootstrapNode` **已删除且不再导出**。
