/**
 * 无 Vercel 账号时的本地 Edge 模拟：用 Node 起 HTTP，转发到 bootstrapVercel 的 fetch。
 */
...
// 勿用 .env 的 PORT（常与 Deno 共用 8000）；本地 Vercel 模拟固定 8001
const port = Number(process.env.EDGE_DEV_PORT ?? 8001);
const host = process.env.HOST ?? "127.0.0.1";

process.chdir(playgroundRoot);

const { bootstrapVercel } = await import("zhin.js/vercel");
...
server.listen(port, host, () => {
  console.log(`[Zhin:vercel-local] http://${host}:${port}/ (pub/health, sandbox-ui)`);
});
