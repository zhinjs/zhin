...
export default async function (args: { city: string }): Promise<string> {
  const city = String(args.city ?? '').trim();
  if (!city) return 'Error: 请提供 city 参数，例如 city=成都';
  ...
  if (!cur) return `未获取到「${city}」的天气数据，请检查城市名是否正确`;
  ...
  return [
    `📍 ${area}`,
    `🌡️ 温度：${cur.temp_C ?? '—'}°C（体感 ${cur.FeelsLikeC ?? '—'}°C）`,
    `☁️ 天气：${desc}`,
    `💧 湿度：${cur.humidity ?? '—'}%`,
    `🌬️ 风速：${cur.windspeedKmph ?? '—'} km/h ${cur.winddir16Point ?? ''}`.trim(),
    `🌧️ 降水：${cur.precipMM ?? '—'} mm`,
