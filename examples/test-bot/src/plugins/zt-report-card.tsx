  ...
  return (
    <div style="display:flex;flex-direction:row;align-items:center;margin-bottom:6px">
      ...
      <div style="display:flex;flex-direction:column;flex:1;min-width:0;gap:4px">
        <div style="color:${T.textMuted};font-size:11px;line-height:14px;font-weight:500;white-space:nowrap">
          {used} / {total}
        </div>
        <UsageBar percent={usage} accent={T.accentDisk} />
      </div>
    </div>
  );
}
...
function buildZtReportBody(data: ZtReportData, subtitle = "Zhin.js test-bot"): string {
  ...
  const sections = [
    ...
    <Section title="处理器">
      ...
      />
      {data.cpuUsage != null ? (
        <MetricBlock label="占用" percent={data.cpuUsage} accent={T.accentCpu} />
      ) : null}
    </Section>,
    <Section title="内存">
      <MetricBlock
    ...
    <Section title="磁盘">
      <MetricBlock
        ...
        accent={T.accentDisk}
      />
      <KvTable rows={[{ label: "IO", value: data.diskIoLine }]} />
      {data.diskMounts.length ? (
        <Col style="margin-top:4px">
          {data.diskMounts.map((d) => (
            <DiskMountRow
              key={d.mount}
              mount={d.mount}
              used={d.used}
        ...
        <MetricBlock label="挂载" value="无分区数据" />
      )}
    </Section>,
    data.gpuLine ? (
      <Section title="GPU">
        <KvTable rows={[{ label: "设备", value: data.gpuLine }]} />
      </Section>
    ) : null,
    <DualSection
      left={{
  ...
  return (
    <Card>
      <CardHeader title="系统状态" subtitle={subtitle} badge={data.hostName} />
      <Row gap={10} style="margin-bottom:6px">
        <StatChip label="CPU" value={formatPercent(data.cpuUsage)} accent={T.accentCpu} />
        <StatChip label="内存" value={formatPercent(data.memUsage)} accent={T.accentMem} />
        <StatChip label="磁盘" value={formatPercent(data.diskUsage)} accent={T.accentDisk} />
      </Row>
      <Col gap={0}>{sections}</Col>
      {data.fallbackNote ? (
        <Surface padding="10px 12px" style="margin-top:14px;">
          <div style={`color:${T.textSecondary};font-size:11px`}>{data.fallbackNote}</div>
        </Surface>
      ) : null}
    </Card>
  );
...
export function buildZtReportHtml(data: ZtReportData, options?: { subtitle?: string }): string {
  return tightHtml(
    <CardCanvas>{buildZtReportBody(data, options?.subtitle)}</CardCanvas>,
  );
}
