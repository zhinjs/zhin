...
function diskMountRow(mount: string, used: string, total: string, usage?: number): string {
  return html`
    <div style="display:flex;flex-direction:row;align-items:center;margin-bottom:6px">
      <div style="display:flex;width:${LABEL_W}px;min-width:${LABEL_W}px;max-width:${LABEL_W}px;color:${T.textMuted};font-size:12px;line-height:16px;text-align:right;padding-right:10px;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${e(mount)}</div>
      <div style="display:flex;flex-direction:column;flex:1;min-width:0;gap:4px">
        <div style="color:${T.textMuted};font-size:11px;line-height:14px;font-weight:500;white-space:nowrap">${e(used)} / ${e(total)}</div>
  ...
  const sections = [
    ...
    h(Section, {
      ...
      children: [
        ...
        data.cpuUsage != null ? h(MetricBlock, { label: "占用", percent: data.cpuUsage, accent: T.accentCpu }) : null,
      ],
    }),
    h(Section, {
      title: "内存",
      children: [
        h(MetricBlock, { label: "物理内存", value: data.memValue, percent: data.memUsage, accent: T.accentMem }),
        data.swapValue
          ? h(MetricBlock, { label: "交换分区", value: data.swapValue, percent: data.swapUsage, accent: T.accentSwap })
          : null,
      ],
    }),
    h(Section, {
      title: "磁盘",
      children: [
        h(MetricBlock, { label: "总用量", value: data.diskValue, percent: data.diskUsage, accent: T.accentDisk }),
        h(KvTable, { rows: [{ label: "IO", value: data.diskIoLine }] }),
        data.diskMounts.length
          ? h(Col, {
              style: "margin-top:4px",
              children: data.diskMounts.map((d) => diskMountRow(d.mount, d.used, d.total, d.usage)),
            })
  ...
  const cardBody = [
    ...
    data.fallbackNote
      ? h(Surface, {
          ...
          children: `<div style="color:${T.textSecondary};font-size:11px">${e(data.fallbackNote)}</div>`,
        })
      : null,
  ];

  return h(Card, { children: cardBody });
}

export function buildZtReportHtml(data: ZtReportData, options?: { subtitle?: string }): string {
  return tightHtml(h(CardCanvas, {
    children: buildZtReportBody(data, options?.subtitle),
  }));
}
