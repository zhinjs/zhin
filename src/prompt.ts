...
export class Prompt {
	 ...
	 */
	async any(message: Element.Fragment = "请输入", timeout_message: Element.Fragment = "输入超时", timeout = this.timeout) {
	 ...
	 */
	text(message: Element.Fragment = "请输入文本", initial = "", options: Omit<Prompt.TextOptions, "type" | "message" | "initial"> = {}) {
		return this.$prompt({
			type: "text",
			message,
			initial,
	 ...
	 */
	number(message: Element.Fragment = "请输入数值", initial = 0, options: Omit<Prompt.NumberOptions, "type" | "message" | "initial"> = {}) {
		return this.$prompt({
			type: "number",
			message,
			initial,
	 ...
	 */
	date(message: Element.Fragment = "请输入日期", initial = new Date(), options: Omit<Prompt.DateOptions, "type" | "message" | "initial"> = {}) {
		return this.$prompt({
			type: "date",
			message,
			initial,
	 ...
	 */
	regexp(message: Element.Fragment = "请输入正则", initial = /.+/, options: Omit<Prompt.RegexpOptions, "type" | "message" | "initial"> = {}) {
		return this.$prompt({
			type: "regexp",
			message,
			initial,
	 ...
	 */
	confirm(message: Element.Fragment = "确认么？", initial: boolean = false, options: Omit<Prompt.ConfirmOptions, "type" | "message" | "initial"> = {}) {
		return this.$prompt({
			type: "confirm",
			message: options.confirm_message ? [
				...(Array.isArray(message) ? message : [message]),
				...(Array.isArray(options.confirm_message) ? options.confirm_message : [options.confirm_message])
	 ...
	 */
	list<T extends keyof Prompt.BaseTypes>(message: Element.Fragment = "请输入", option: Prompt.Option<"list", T>) {
		return this.$prompt({
			type: "list",
	 ...
	 */
	select<T extends keyof Prompt.BaseTypes, M extends boolean>(message: Element.Fragment = "请选择", option: Prompt.Option<"select", T, M>) {
		const options: Prompt.Option<"select", T, M> = {
			...
			format: (event) => {
				...
				return Prompt.transforms["select"][option.child_type][option.multiple ? "true" : "false"](event, option.options, choiceArr) as Prompt.Select<T, M>;
			}
		};
...
export namespace Prompt {
	...
	export type SelectOptions<T extends keyof BaseTypes, M extends boolean> = Option<"select", T, M>
