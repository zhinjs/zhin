...
import { Zhin, Schema, useContext } from "@";
...
const root = ctx.useOptions(
    "plugins.watcher",
    Schema.string().default(process.cwd()).required().description("监听路径"),
);
...
watcher.on("change", filename => {
    if (path.resolve((process.env.configPath ||= "zhin.yaml")) === filename) {
        const newOptions: Zhin.Options = Yaml.load(
            fs.readFileSync(process.env.configPath, "utf8"),
        ) as any;
