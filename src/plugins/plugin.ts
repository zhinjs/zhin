...
ctx.command("plugin [action:string] [name:string]")
    ...
    .action<NSession<keyof Zhin.Adapters>>(({ session }, action, name) => {
        ...
        if (!name) return session.execute(`plugin.${action}`);
            ...
            return (
                plugins
                    .map((o, idx) => {
                        ...
                        let enableStatus = installStatus
                            ? getPluginStatus(ctx, session, o.fullName)
                            : "□";
            ...
            return (
                packages
                    .map((o, idx) => {
                        ...
                        const installVersion = localPlugins.find(p => p["fullName"] === o.name)?.[
                            "version"
                        ];
                        const isLatest = installVersion === o.version;
                        return `${installStatus ? "●" : "○"} ${o.name}@${o.version}${
                            isLatest ? " (latest)" : installStatus ? `(${installVersion})` : ""
                        }`;
                    })
                    .join("\n") + `\n\n●:已安装 ○:未安装`
            );
...
ctx.master()
    ...
    .action<NSession<keyof Zhin.Adapters>>(async ({ session, options }, name) => {
        ...
        try {
            const [success, err] = await changeDependency(
                `${name}@${options.version || info.version}`,
            );
...
ctx.master()
    ...
    .action<NSession<keyof Zhin.Adapters>>(async ({ session }, name) => {
        ...
        if ([Plugin.Source.built, Plugin.Source.local].includes(options.type))
            return "本地/内置不允许卸载";
...
ctx.role("master", "admins")
    ...
    .desc("移除插件")
    .action(({ session }, name) => {
        const plugin = ctx.zhin.pluginList.find(
            p => p.options.fullName === name || p.options.name === name,
        );
...
ctx.command("plugin/plugin.detail <name:string>")
    ...
    .action(async ({ options }, name) => {
        ...
        return JSON.stringify(plugin.info, null, 2).replace(/"/g, "").replace(/\\/g, "");
    });
