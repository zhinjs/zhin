...
ctx.master()
	...
	.action<NSession<keyof Zhin.Adapters>>(async ({ options, session }) => {
		if (!options.version) options.version = await ctx.request.get("https://registry.npmjs.org/zhin")
			.catch(() => ({ "dist-tags": { "latest": "0.0.0" } }))
			.then(({ distTags }) => distTags.latest);
		if (options.version === ctx.zhin.version) return "无需更新";
		const existVersions = await ctx.zhin.request.get("https://registry.npmjs.org/zhin")
			.catch(() => ({ "versions": {} }))
		...
		if (!options.restart) return "更新成功，将在下次重启生效。\n更新内容见：https://github.com/zhinjs/zhin/blob/main/CHANGELOG.md";
		const channelId = Bot.getFullTargetId(session);
		process.send({
			type: "queue", body: {
				channelId,
				message: "以更新并重启。更新内容见：https://github.com/zhinjs/zhin/blob/main/CHANGELOG.md"
			}
		});
