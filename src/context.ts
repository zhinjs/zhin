...
export class Context extends EventEmitter {
	...
	async beforeReady<T>(callback: () => T | Promise<T>): Promise<T> {
		...
		return new Promise<T>(resolve => this.zhin.on("before-ready", () => {
			resolve(callback());
		}));
	}
	...
	async afterReady<T>(fn: () => T | Promise<T>): Promise<T> {
		...
		return new Promise<T>(resolve => this.zhin.on("after-ready", () => {
			resolve(fn());
		}));
	}
	...
	async beforeStart<T>(callback: () => T | Promise<T>): Promise<T> {
		...
		return new Promise<T>(resolve => this.zhin.on("before-start", () => {
			resolve(callback());
		}));
	}
	...
	async afterStart<T>(callback: () => T | Promise<T>): Promise<T> {
		if (this.zhin.isStarted) return callback();
		return new Promise<T>(resolve => this.zhin.on("after-start", () => {
			resolve(callback());
		}));
	}
