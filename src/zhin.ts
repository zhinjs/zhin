...
export class Zhin extends Context {
    ...
    constructor(options: Zhin.Options) {
        ...
        const result = (this.zhin = new Proxy(this, {
            get(target: Zhin, p: string | symbol, receiver: any): any {
                if (target.services.has(p as keyof Zhin.Services))
                    return target.services.get(p as keyof Zhin.Services);
        ...
        if (fs.existsSync(path.join(process.cwd(), "permissions.yaml"))) {
            this.permissions = Yaml.load(
                fs.readFileSync(path.join(process.cwd(), "permissions.yaml"), {
                    encoding: "utf-8",
                }),
    ...
    static getChannelId(event: Dict) {
        return [
            event.message_type,
            event.group_id || event.discuss_id || event.sender.user_id,
        ].join(":") as ChannelId;
    }
    ...
    async getMarketPackages(): Promise<Plugin.Package[]> {
        const { objects: modules } =
            (await this.services
                .get("request")
                .get(
                    "https://registry.npmjs.org/-/v1/search?text=keywords:zhin%20zhin-plugin&size=250",
                )) || {};
    ...
    public load<T extends Zhin.ModuleCategory>(
        ...
        function getListenDir(modulePath: string) {
            if (modulePath.endsWith(path.sep + "index"))
                return modulePath.replace(path.sep + "index", "");
        ...
        const resolved = getModulesPath(
            [
                this.options[`${category}_dir`]
                    ? path.resolve(this.options[`${category}_dir`], name)
                    : null, // 用户自定义插件/服务/游戏目录
        ...
        if (packageInfo?.name) {
            packageInfo.name = packageInfo.name.replace(
...
export function createWorker(options: Zhin.WorkerOptions) {
    ...
    const forkOptions: ForkOptions = {
        ...
        execArgv: ["-r", "esbuild-register", "-r", "tsconfig-paths/register"],
        stdio: "inherit",
    };
