...
function ownership(): SourceOwnershipIndex {
  ...
  index.add({
    source: '/project/plugins/child/commands/status/index.ts',
    role: 'capability',
    owner: child,
  ...
  index.add({
    source: '/project/plugins/sibling/commands/status/index.ts',
    role: 'capability',
    owner: sibling,
...
describe('InvalidationPlanner', () => {
  it('keeps a direct Capability change at Slot scope', () => {
    const plan = new InvalidationPlanner(ownership()).plan([
      '/project/plugins/child/commands/status/index.ts',
    ]);
  ...
  it('uses reverse importers to classify an otherwise untracked support module', () => {
    const dependencies: DependencyImpactPort = {
      affectedSources: () => ['/project/plugins/child/commands/status/index.ts'],
    };
