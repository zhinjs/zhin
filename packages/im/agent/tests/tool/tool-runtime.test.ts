...
describe('ToolRuntime', () => {
  it('executes a tool and returns output + durationMs', async () => {
    const rt = new ToolRuntime(makeCtx());
  ...
  it('does not fabricate IM authority for standalone execution', async () => {
    const rt = new ToolRuntime(makeCtx({ commMessage: undefined }));
    const tool = makeTool({
  ...
  it('rejects when tool generation mismatches turn generation', async () => {
    const rt = new ToolRuntime(makeCtx({ generation: 2 }));
  ...
  it('allows execution when tool.generation matches turn generation', async () => {
    const rt = new ToolRuntime(makeCtx({ generation: 3 }));
  ...
  it('skips generation check when tool.generation is undefined', async () => {
    const rt = new ToolRuntime(makeCtx({ generation: 5 }));
  ...
  it('throws when signal is already aborted', async () => {
    ...
    const rt = new ToolRuntime(makeCtx({ signal: ac.signal }));
    const tool = makeTool();
  ...
  it('emits journal events for successful execution', async () => {
    ...
    const rt = new ToolRuntime(makeCtx({ journal }));
  ...
  it('exposes generation on the runtime instance', () => {
    const rt = new ToolRuntime(makeCtx({ generation: 42 }));
