...
describe('Setup - Default Configuration', () => {
  ...
  it('should generate default config when file does not exist', async () => {
    ...
    expect(config.plugins).toEqual(['@zhin.js/host-router', '@zhin.js/host-api', '@zhin.js/adapter-sandbox'])
  ...
  it('should have correct plugins loading order', async () => {
    ...
    expect(config.plugins).toContain('@zhin.js/host-api')
    expect(config.plugins).toContain('@zhin.js/adapter-sandbox')
    
    // 验证顺序
    const httpIndex = config.plugins.indexOf('@zhin.js/host-router')
    const consoleIndex = config.plugins.indexOf('@zhin.js/host-api')
