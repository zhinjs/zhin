...
共享 group/channel session 的 `UserMessage.actor` 是参与者身份的唯一权威，保存稳定
`subjectId`、显示名与角色；`agent_messages.extra` 只保存引用等展示上下文。LLM 边界从 actor
渲染参与者标签，Compaction transcript 也直接读取 actor，并要求摘要保留决定、约束、TODO、
分歧及 authority 来源的归属。
