...
async function relaunchWithNativeTypeScript(): Promise<boolean> {
  ...
  if (major < 22 || (major === 22 && minor < 6)) {
    throw new Error(
      `zhin-next start requires Node >=22.6.0 for native TypeScript; found ${process.versions.node}`,
    );
  }
  const entry = process.argv[1];
  if (!entry) throw new Error('Cannot determine the zhin-next executable path');
