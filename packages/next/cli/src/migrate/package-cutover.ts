...
/** Builds a Next manifest without mutating legacy source or its current entrypoint. */
export class PackageCutover {
  async plan(projectRoot: string): Promise<PackageCutoverPlan> {
    const root = resolve(projectRoot);
    const packageFile = join(root, 'package.json');
    const entryFile = join(root, 'plugin.next.ts');
    ...
    if (await usesCompat(root, capabilities)) dependencies['@zhin.js/next-compat'] = '^0.0.0';
...
async function usesCompat(root: string, capabilities: readonly CutoverCapability[]): Promise<boolean> {
  for (const capability of capabilities) {
    if (await directoryContainsText(
      join(root, capabilityDirectories[capability]),
      "from '@zhin.js/next-compat'",
    )) return true;
  }
  return false;
}

async function directoryContainsText(directory: string, search: string): Promise<boolean> {
  let entries;
  try { entries = await readdir(directory, { withFileTypes: true }); }
  catch { return false; }
  for (const entry of entries) {
    const path = join(directory, entry.name);
    if (entry.isDirectory() && await directoryContainsText(path, search)) return true;
    if (entry.isFile() && /\.tsx?$/u.test(entry.name)
      && (await readFile(path, 'utf8')).includes(search)) return true;
  }
  return false;
}
...
function isCompletedManifest(value: unknown): value is Record<string, unknown> {
  return !!value && typeof value === 'object'
    ...
    && (value as Record<string, unknown>).entry === './plugin.next.ts';
}
...
function assertPlanPaths(plan: PackageCutoverPlan): void {
  if (plan.packageFile !== join(plan.root, 'package.json')
    || plan.entryFile !== join(plan.root, 'plugin.next.ts')) {
