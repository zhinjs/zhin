...
所有新建 Root、child Plugin 与 Feature package 都声明 `zhin.engine: ^1.0.0`；Feature 同时声明具体 `featureApi: 1.0.0`，调用方 requirement 使用 `^1.0.0`。Root tsconfig 覆盖 Command、Middleware、Component 以及一级 `plugins/*`、`packages/*` 的 TypeScript/TSX 源码。
