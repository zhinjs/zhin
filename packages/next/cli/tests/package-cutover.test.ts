...
import { PackageCutover } from '../src/migrate/index.js';
...
describe('package cutover', () => {
  ...
  it('rejects conflicting prepared entries and existing manifests', async () => {
    const root = await fixture();
    await writeFile(join(root, 'plugin.next.ts'), 'different');
...
async function fixture(extra: Record<string, unknown> = {}): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'zhin-next-cutover-'));
  temporary.push(root);
  await Promise.all([
...
function legacyCommandDefinition(): string {
  return "import { defineLegacyCommand } from '@zhin.js/next-compat';\n";
}

function legacyMiddlewareDefinition(): string {
  return "import { defineLegacyMiddleware } from '@zhin.js/next-compat';\n";
}
