...
`DiscoveryHost` 是唯一 I/O 边界：`list()` 枚举目录，`loadModule()` 加载 Server TS/JS 模块，`readText()` 加载 Markdown/JSON 文本。可选 `loadClientModule()` 只返回 Page/Layout 等浏览器源码的静态 build artifact，不得在 Node 中执行它们。默认 Runtime 可提供预编译 ESM host；开发期 transform、client compiler 和 watcher 应由独立 adapter 实现。
