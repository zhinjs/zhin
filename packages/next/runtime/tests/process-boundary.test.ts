...
describe('Root/process boundary', () => {
  ...
  it('escalates a mounted Feature API contract change', async () => {
    const project = await createProject();
    await writeFeatureRootManifest(project);
    ...
    await hmr.enqueue(join(project, 'packages/command/package.json'));
...
async function writeFeatureRootManifest(root: string): Promise<void> {
  await writeJson(join(root, 'package.json'), {
    name: '@test/root',
    type: 'module',
    ...
    zhin: {
      ...
      entry: './plugin.ts',
      runtime: 'trusted',
      features: [{ package: '@test/command', api: '^1.0.0' }],
    },
  });
}
