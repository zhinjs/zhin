...
拓扑只来自静态 manifest，不从 `plugin.ts` 的执行副作用中发现。`runtime: isolated` child 需要显式安装并传入 [`@zhin.js/next-isolate`](../isolate/README.md)；Root Plugin 只能使用 trusted runtime。

隔离适配器只收到 entry、owner config 和 RuntimeEnvironment，不收到 Host Scope。隔离 Plugin 不能 mount 普通 Host Feature provider；跨边界 Feature 必须先定义具体 codec/proxy contract。Worker/process 提供故障与并发隔离，不等同于执行不可信代码的安全沙箱。
...
| `ModuleRuntime.loadClientModule` | 可选 Page/Layout build artifact port，不在 Node 执行 client source |
| `RootResourceInstaller` | 向 Root Scope 安装数据库、配置存储等共享 Resource |
| `IsolatedPluginRuntimePort` | 可选 child Plugin Worker/process 生命周期与 generation handoff port |
