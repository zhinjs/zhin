...
Console Shell Layout slot 的标准 Feature provider。Layout 与 Page 共用 `pages/`，但只发现两个保留目录：`nav/` 和 `footer/`。

## Semantics

- `pages/nav/index.tsx` / `index.ts` -> slot `nav`。
- `pages/footer/index.tsx` / `index.ts` -> slot `footer`。
