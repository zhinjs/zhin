...
export function registerEntriesRoute(
  ...
  registerFetchRoute(router, "GET", "/entries", (ctx: RouterContext) => {
    ctx.body = getBody();
  });
}
...
export function registerHostRestRoutes(
  ...
  registerSystemStatusRoute(router, base);
  ...
  registerFetchRoute(router, "GET", `${base}/plugins/:name`, async (ctx: RouterContext) => {
    const root = getRoot();
  ...
  registerFetchRoute(router, "GET", `${base}/config/:name`, async (ctx: RouterContext) => {
    const root = getRoot();
  ...
  registerFetchRoute(router, "POST", `${base}/config/:name`, async (ctx: RouterContext) => {
    const root = getRoot();
    const { name } = ctx.params;
    if (name === "app") {
  ...
  registerFetchRoute(router, "POST", `${base}/message/send`, async (ctx: RouterContext) => {
    const root = getRoot();
