...
export class Command<A extends any[] = [], O = {}> {
  ...
  //显示帮助信息
  help({ simple, showHidden, dep = 1, current = 0 }: HelpOptions = {}, allowList: Command[] = []): string[] {
    ...
    if (!simple) {
      ...
      if (this.sugarsConfig.length) output.push(` 语法糖:${this.sugarsConfig.map(sugar => String(sugar.regexp))}`);
      if (!isEmpty(this.optionsConfig)) {
        const options = Object.keys(this.optionsConfig).filter(name => !name.startsWith('-'));
        if (options.length) {
          output.push(' 可选项:');
          options.forEach(key => {
            const nameDesc: string[] = [];
    ...
    for (const sugar of this.sugarsConfig) {
      ...
      for (let i = 0; i < args.length; i++) {
        ...
        if (typeof arg === 'string' && arg.startsWith('$') && matched[+arg.slice(1)]) {
          args[i] = arg = Command.transform([matched[+arg.slice(1)]], argConfig.type as Command.Type);
        }
        ...
        if (!argConfig.rest) argv.args[i] = arg;
          ...
          argv.args[i] = Command.transform([arg, ...args], argConfig.type as Command.Type);
        }
      }
      for (const option of Object.keys(options)) {
        ...
        if (
          ...
          options[option] = Command.transform([matched[+options[option].slice(1)]], optionConfig.type as Command.Type);
        }
        const isValid = Command.validate(optionConfig.type, optionConfig.rest, options[option]);
    ...
    while (matchedArr.length) {
      ...
      if (arg.startsWith('--') || arg.startsWith('-')) {
        const name = arg.startsWith('--')
          ? arg.slice(2)
          : Object.entries(this.optionsConfig).find(([, option]) => option.name === arg.slice(1))?.[0];
...
export namespace Command {
  ...
  export function checkArgv(argv: Argv, argsConfig: ArgConfig[], optionsConfig: OptionsConfig) {
    ...
    for (const option in optionsConfig) {
      ...
      const validate =
        optionConfig.type && domains[optionConfig.type] && domains[optionConfig.type as keyof Command.Domain]?.validate;
  ...
  export function validate<T extends Type>(type: string, rest: boolean, value: Domain[T] | Domain[T][]): boolean {
  ...
  export function registerDomain<T extends Type, A extends string[]>(
    type: T,
    config:
      | (Pick<DomainConfig<T, A>, 'transform'> & Partial<Omit<DomainConfig<T, A>, 'transform'>>)
      | ((...source: A) => Domain[T]),
  ) {
