...
export async function loadPlugins(plugin: Plugin, appConfig: AppConfig): Promise<void> {
  ...
  const appRoot = getZhinProjectRoot();
  plugin.logger.debug(`Plugin list: ${Array.from(pluginNames).join(', ')} (projectRoot=${appRoot})`);

  const dirs = appConfig.plugin_dirs || [];
  for (const pluginName of pluginNames) {
    const dir = dirs.find((d: string) => resolveEntry(path.join(appRoot, d, pluginName)));
    if (dir) {
      const pluginPath = path.join(appRoot, dir, pluginName);
