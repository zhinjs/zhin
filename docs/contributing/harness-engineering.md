...
`check:all` **已包含** `pnpm type-check`、`pnpm lint`、`pnpm test`，本地一条命令即可覆盖类型/风格/单测与架构 harness。

CI 若需单独跑 coverage，可设 `HARNESS_SKIP_TEST=1` 跳过其中的 unit tests，避免与 coverage 作业双跑。
...
pnpm check:agent-tool-schema     # agent/tools inputSchema 与 defineAgentTool/execute 字段一致
pnpm check:stable           # Stable smoke（Sandbox + minimal-bot 契约，无 LLM）
pnpm check:l4-ci            # PR 门禁 L4 子集
pnpm type-check             # 也可单独跑（已含于 check:all）
pnpm lint
pnpm test
```
...
1. **本地开发** — 运行 `pnpm check:all`（含 type-check / lint / test + 架构 harness）
2. **提交前** — 同上；勿只跑部分脚本而漏掉类型或单测
