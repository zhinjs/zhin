...
Persistent Workroom Catalog entries may list `sponsors[]` as authenticated principal ids (for example, `owner:<platform-user-id>`). This membership—not message metadata—authorizes typed Project Sponsor controls and is pinned by the exact Catalog/Project digest carried by a Plan.
