...
After a normal Workroom discussion is durably recorded in the Project Inbox, a one-shot handoff continues into the Catalog Orchestrator turn and returns the Agent response instead of stopping at an inbox receipt. Only an explicit `/work` request enters governed planning and may create a Run or Tasks.
