...
:::
zhin为开发者提供了两种方式来新建一个插件，分别是通过`cli`和`手动创建`。推荐使用`cli`创建，因为这样可以省去很多重复的工作。现在我们就来看看如何创建一个插件。
### 1. cli创建
...
```
:::
现在，你可以给bot发送`test -f hello`来测试一下了（如果对应bot有配置prefix，需要在发送的指令前边加上对应prefix）
### 2.2 定义中间件
::: code-group
```javascript [javascript]
// index.js
