...
当前 `@zhin.js/next-feature-component` 已实现 Feature 部分：递归发现 `components/**/*.ts|tsx`，从 requester exact owner 向 Root 做 ancestor fallback。render context 的 owner config/resource 属于实际声明者，并单独保留 requester identity。它不依赖 React 或 IM SendContent；Outbound Renderer 会在后续 IM Runtime 切片把结果接入统一发送链路。
