...
当前 Runtime 已提供内存 document 的 `ConfigPatchPlanner` 与 `RootRuntime.patchConfig()`。Planner 接收结构化 `set/remove` 操作，在 materialized document clone 上应用后重新执行整树 schema 校验，并通过前后 owner ConfigView 的结构化比较计算最浅 forest。patch 调用自身串行化；校验或 shadow setup 失败不更新 document，相同语义不发布 generation。YAML AST 持久化仍属于后续 `ConfigDocumentPort` adapter。
...
- 相同值、default 抵消与删除不存在路径不发布空 generation。
