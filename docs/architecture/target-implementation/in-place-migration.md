...
- 已完成：Plugin Runtime、Feature Kit、IM/Agent 领域 Feature、Core IM Runtime，以及
  `@zhin.js/agent/runtime` 的 snapshot-coherent turn 入口。
- 兼容边界：旧 Agent mutable ingress 暂作为适配器保留；Stable Root 切到 RuntimeSnapshot 后删除。
- 下一批：Console contract、Page/Layout Feature 与 Console Runtime 原位归属。
