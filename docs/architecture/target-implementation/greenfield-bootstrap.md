...
下一阶段进入 Agent 能力纵向切片：Tool、Skill、Agent、MCP Feature 与 CapabilityIngress；随后推进 Page/Layout manifest、CLI publish recovery 和隔离/HMR 硬化。
...
- Page/Layout、Agent、Skill、Tool 是后续独立 Feature package，不回填 Kernel 枚举；Middleware、Component、Adapter 与 IM Runtime 已实现。
...
|---|---:|---|
| 绿地建设 | 3–5 | Agent/Tool/Skill/MCP；Page/Layout；CLI 发布恢复；隔离与 HMR 硬化 |
...
当前绿地底座完成度约 76%，把迁移与稳定化计入后的整体替换完成度约 45%。按当前批量推进节奏，还需约 13–22 个 focused stages；若持续一次完成多个相邻切片，代码迁移仍可争取在约 3–5 周收口，达到可替换稳定版本更现实的区间是 5–8 周。真实 IM 平台验收、外部插件反馈或兼容范围扩大时应更新本表，不压缩测试来追日期。
