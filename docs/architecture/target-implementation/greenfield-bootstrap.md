...
| `@zhin.js/next-runtime` | Manifest parser、workspace/npm resolver、ProjectGraph、ConfigComposer、RuntimeEnvironment/owner EnvStore、RootRuntime、ModuleRuntime、source ownership、HMR 与 process restart |
...
下一阶段进入协议兼容门禁：校验 Plugin `engine`、Feature requirement `api` 与 provider `featureApi`，让不兼容 package 在任何 setup/discovery 副作用前失败。
...
- `engine`、Feature requirement `api` 与 provider `featureApi` 已进入 protocol，semver compatibility gate 是下一阶段。
...
|---|---:|---|
| 绿地建设 | 5–7 | 协议兼容门禁；IM Middleware/Component/Adapter；Agent/Tool/Skill；Page/Layout；CLI 与发布恢复；隔离/HMR 硬化 |
| 旧代码迁移 | 7–12 | compatibility facade/codemod；Core/Agent/Console；Adapters；services/utils；examples、CLI 与文档入口切换 |
| 稳定化 | 3–5 | 双版本对照、真实平台验收、性能/体积、安全门禁、release candidate |

当前绿地底座完成度约 60%，把迁移与稳定化计入后的整体替换完成度约 35%。按当前每次推进一个完整 stage 的节奏，还需约 15–24 次 focused stages；若保持近乎每日推进，代码迁移可在约 3–5 周收口，达到可替换稳定版本更现实的区间是 5–8 周。真实 IM 平台验收、外部插件反馈或兼容范围扩大时应更新本表，不压缩测试来追日期。
