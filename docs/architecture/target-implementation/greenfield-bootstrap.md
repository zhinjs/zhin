...
下一阶段继续完成 IM Runtime：Adapter/Endpoint Resource、Inbound Runner、Command Dispatcher、Component Outbound Renderer 与统一 send hook 链路。
...
- Page/Layout、Agent、Skill、Tool 是后续独立 Feature package，不回填 Kernel 枚举；Middleware、Component 已实现。
...
|---|---:|---|
| 绿地建设 | 4–6 | IM Adapter/Endpoint/统一发送；Agent/Tool/Skill；Page/Layout；CLI 发布恢复；隔离与 HMR 硬化 |
...
当前绿地底座完成度约 68%，把迁移与稳定化计入后的整体替换完成度约 40%。按当前批量推进节奏，还需约 14–23 个 focused stages；若持续一次完成多个相邻切片，代码迁移仍可争取在约 3–5 周收口，达到可替换稳定版本更现实的区间是 5–8 周。真实 IM 平台验收、外部插件反馈或兼容范围扩大时应更新本表，不压缩测试来追日期。
