...
下一阶段进入可选 Client Module AST/build adapter 与 CLI publish recovery；随后推进隔离/HMR 硬化。
...
- Page/Layout 独立 Feature、Console contract/runtime 已实现；TSX 到 browser chunk/manifest 的可选 build adapter 尚未实现。
...
|---|---:|---|
| 绿地建设 | 2–3 | Client build adapter；CLI 发布恢复；隔离与 HMR 硬化 |
...
当前绿地底座完成度约 89%，把迁移与稳定化计入后的整体替换完成度约 53%。按当前批量推进节奏，还需约 11–20 个 focused stages；若持续一次完成多个相邻切片，代码迁移仍可争取在约 3–5 周收口，达到可替换稳定版本更现实的区间是 5–8 周。真实 IM 平台验收、外部插件反馈或兼容范围扩大时应更新本表，不压缩测试来追日期。
