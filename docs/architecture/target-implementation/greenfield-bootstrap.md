...
下一阶段进入 `runtime: isolated` Worker/process adapter 与 HMR 硬化，然后开始 compatibility facade/codemod 与旧代码迁移。
...
|---|---:|---|
| 绿地建设 | 1–2 | isolated runtime adapter；隔离/HMR 硬化 |
...
当前绿地底座完成度约 94%，把迁移与稳定化计入后的整体替换完成度约 56%。按当前批量推进节奏，还需约 10–19 个 focused stages；若持续一次完成多个相邻切片，代码迁移仍可争取在约 3–5 周收口，达到可替换稳定版本更现实的区间是 5–8 周。真实 IM 平台验收、外部插件反馈或兼容范围扩大时应更新本表，不压缩测试来追日期。
