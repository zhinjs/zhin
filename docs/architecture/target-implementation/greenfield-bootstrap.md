...
下一阶段继续收紧环境边界：定义 owner-scoped `EnvStore` 与显式 overlay 顺序，使 `${ENV}` 的保留、解析、secret redaction 和环境差异不混入 YAML adapter 或 Plugin 全局状态。
...
- YAML ConfigDocument adapter 已实现；环境 overlay、类型化 EnvStore 与 secret redaction 尚未实现。
