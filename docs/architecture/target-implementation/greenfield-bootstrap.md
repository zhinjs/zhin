...
Stable `minimal-bot` 与厨房水槽 `test-bot`（默认 `zhin runtime start`）均已切正式 Root；
平台 Adapter / utils / services / games 约定式 cutover 已齐（见
[in-place-migration.md](./in-place-migration.md)）；`migration-topology.json` 的
`pending` 为空。余量转为 **Agent Host 行为对齐**、**Console/full-bot 收口**与**稳定化**。
...
- 仍待：Plugin 发布 JS entry / manifest artifact；Agent Host 与 legacy `createZhinAgent` 全栈对齐；`full-bot` 切 Runtime；Console `db:*` 真接线。

## 5. 建设与迁移余量

以下估算以“一次 focused stage 完成设计、代码、测试、README、SSOT 和提交”为单位，不把只写接口视为完成（**2026-07 更新**）：

| 区段 | 剩余 focused stages | 主要内容 |
|---|---:|---|
| 绿地建设 | 0 | API snapshot 与隔离并发 soak 已完成，公共面进入冻结 |
| 结构 / 包迁移 | 0–1 | 拓扑 `pending: []`；仅余发布 manifest / JS entry 边角 |
| 行为对齐 | 1–2 | Console `db:*`；`full-bot` 切 Runtime（**Agent Host P0 已收口**） |
| 稳定化 | 2–4 | 双版本对照、真实平台验收、性能/体积、安全门禁、release candidate；删 legacy 入口 |

| 口径 | 完成度 |
|---|---:|
| 绿地底座 | ~100% |
| 结构 cutover（包 + Adapter + 约定式 Plugin） | ~95% |
| 含厨房水槽主路径行为对齐 | ~92–95% |
| 含稳定化可替换版 | ~82–86% |

按当前节奏，行为对齐约 **2–4 周**可收口主缺口；达到可替换稳定版本更现实的区间是 **4–6 周**。真实 IM 平台验收、外部插件反馈或兼容范围扩大时应同步更新本表与
[in-place-migration.md](./in-place-migration.md) 的「剩余缺口」节，不压缩测试来追日期。
