...
2. **轻量**：生产 Kernel 不携带 YAML、JSON Schema compiler、编译器、React、AI SDK。
...
“轻量”不是拒绝成熟依赖，而是让依赖只出现在真正需要它的 adapter 中。JSON Schema 应使用成熟 compiler，TS/TSX HMR 应使用独立 module adapter；它们不进入 `@zhin.js/plugin-runtime` 或 `zhin.js` 默认生产闭包。
...
flowchart TB
  ...
  ConsoleProtocol["@zhin.js/console-protocol\nRPC names / normalization / scope policy"]
  ConsoleProtocol --> ConsoleClient
  ...
  Dev["Optional dev ModuleRuntime\nseparate install budget"] -. development only .-> Zhin
```

### 2.1 `@zhin.js/plugin-runtime`
...
不再放在 Kernel：YAML/TOML、Schema 实现、Schedule engine、IM 类型、文件 watcher、compiler/bundler。
...
- `@zhin.js/console-protocol`：零依赖的 RPC/推送名称、兼容归一化与 demo scope policy 单一事实源。
...
- 页面 compiler/bundler 只属于 dev/build adapter，生产 Host 读取预构建 manifest。

Console contract 不应依赖 `@zhin.js/ai`；Agent stream contract 由可选 subpath 或中立 transport contract 提供。

> 当前迁移状态：`@zhin.js/contract` 根入口仍为兼容旧调用方重导出 Agent stream，因此暂时依赖 `@zhin.js/ai`；这不是目标依赖方向。`@zhin.js/console-protocol` 已由 `check:architecture` 强制为零依赖，新的 Endpoint wire contract 只放在 protocol 中。
...
```text
packages/im/plugin-runtime/src/
├── identity.ts
├── token.ts
├── scope.ts
├── capability.ts
...
| 当前代码 | 目标处理 |
|---|---|
| [`kernel/src/plugin.ts`](../../../packages/im/kernel/src/plugin.ts) | 旧 PluginBase 仅作兼容；新树生命周期已迁入 `@zhin.js/plugin-runtime` |
| [`kernel/src/feature.ts`](../../../packages/im/kernel/src/feature.ts) | 旧可变 Feature 逐步改为向 RuntimeSnapshot 投影的兼容 Adapter |
| [`core/src/plugin.ts`](../../../packages/im/core/src/plugin.ts) | IM authoring definition 与 Runtime 留在 Core；Plugin lifecycle 下沉 Plugin Runtime |
...
|---|---:|---|
| `@zhin.js/plugin-runtime` | 0 | 平台内建能力 + 类型 |
...
| dev ModuleRuntime adapter | ≤2.5MB 独立安装 | 不进入默认闭包；禁止无关 CSS/前端构建依赖与大型 native binary |
...
- `@zhin.js/plugin-runtime/package.json` 禁止任何第三方 runtime dependency。
- `@zhin.js/contract` 禁止依赖 React、AI、Core 和 Node-only 包。
- `@zhin.js/core` 禁止依赖 Agent、Console、Host、compiler/bundler、数据库 driver。
