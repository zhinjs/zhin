...
Accepted — **Amended 2026-07**（D2/D3 与 `outbound-resolver` SSOT，见下文）
...
**全局输出约定（Amended）**：协作 turn system prompt 追加统一 JSON outbound 契约（`mentions` + `text`），与 `structured_only` 范围互补；见 `COLLABORATION_REPLY_JSON_HINT`。

### D3. 解析 fallback 链（协作 Cell）

1. JSON parse → `mentions` → **canonical `mention` segment**（主路径）
2. JSON 失败 → 正文假 `@role` / `@endpointId` 兜底重写
3. 仍失败 → **剥离假 `@` token**、仅发 prose + `stage: ai_outbound` warn + `peerTriggerExpected: false`
...
- Agent SSOT：**[`outbound-resolver.ts`](../../packages/im/agent/src/collaboration/outbound-resolver.ts)**（parse → planner role strategy → fallback → expand）
- Agent：`structured-ai-outbound.ts` 负责结构化 parse；planner role strategy 内聚于 `outbound-resolver.ts`
- 协作迁移：[`collaboration-outbound.ts`](../../packages/im/agent/src/collaboration/collaboration-outbound.ts) 委托 core DSL；resolve 路径仅 canonical `mention`（废弃 legacy `at`）
