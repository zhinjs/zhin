...
插件包根目录的 `pages/` 下，只有 `<name>/index.tsx` / `index.ts` 是页面：**默认导出 React 组件**，并用命名导出 `meta` 声明元数据。页面目录内其他文件是 helper，不会成为页面：
...
| `pages/workroom/index.tsx` | root（应用） | `/p-workroom` |
| `pages/index/index.tsx` | root（应用） | `/` |

即：`index` 映射到插件路径本身（不带叶子段），其它文件映射为 `p-<name>` 叶子。路由冲突（两个页面算出同一路由）在启动期报错。

## 布局（@zhin.js/layout）

`pages/` 下有两个保留文件名提供布局槽位：`pages/nav/index.tsx` 对应 `nav` 槽（导航区），`pages/footer/index.tsx` 对应 `footer` 槽（页脚区）。布局文件默认导出 React 组件即可，不需要 `meta`；同一槽位同时存在 `.ts` 与 `.tsx` 时以 `.tsx` 为准。
...
```mermaid
flowchart LR
    A["pages/*/index.tsx<br/>meta = definePage()"] --> B["TypeScriptClientBuilder<br/>(esbuild 打包)"]
...
两个 Feature 各管一摊：`@zhin.js/adapter` 发现 `adapters/` 下的适配器（WebSocket `/sandbox` Endpoint）；`@zhin.js/page` 发现 `pages/index/index.tsx`，于是 Console 里出现 **`/sandbox` 聊天页**。`SandboxChat` 组件通过 WebSocket 连到 Host 的 `/sandbox`（base 与 token 见 `pages/sandboxTransport.ts`），收发消息走统一的 IM 链路——在页面里发消息等价于一个真实平台的入站消息，会经过中间件、命令匹配、AI 未命中处理。
