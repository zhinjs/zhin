...
| Slack | reaction, message | reaction | 无 typing API；`supportsTyping: false`，默认用表情回应 |
...
useContext('icqq', (adapter) => {
  for (const [endpointId, bot] of adapter.endpoints.entries()) {
...
1. Endpoint 连接时，检查配置中是否有 `typingIndicator` 配置
