...
启用 `assistant.enabled: true` 后，Cron 类任务以 **`data/assistant-jobs.json`** 为 SSOT；启动时自动从 `cron-jobs.json` / `scheduler-jobs.json` 迁移（仅当 assistant-jobs 为空）。可选 **`legacyDualWrite: true`** 会同步写回 `cron-jobs.json`（默认关闭）。

```yaml
assistant:
  enabled: true
  legacyDualWrite: false  # 默认；迁移期可 true
  defaults:
    notify:
      channel: im
      platform: icqq
...
持久化 Job **必须**包含 `notify` 字段（JSON 示例）：
...
静默任务：`"notify": { "channel": "silent" }`。旧版 `context` 字段**不再读取**。

`cron_add` / `cron_list` 等 AI 工具与 `zhin cron` CLI 在启用后读写同一 JobStore。详见 [Assistant Runtime 路线图](/architecture/assistant-runtime)。

### 投递通道（M3 NotificationRouter）

Job 结果通过 `notify` 字段投递，不再依赖扁平 `context`：

| `notify.channel` | 行为 |
|------------------|------|
| `im` | `Adapter.sendMessage`（走 ADR 0004 队列契约） |
| `silent` | 仅更新 Job 状态，不发送 |
| `log` | 写入应用日志 |
| `ha` | Home Assistant 服务（M4 细化） |

`cron_add` 参数 **`notify_channel`**：`im`（默认）、`silent`、`log`。`cron-jobs.json` / `assistant-jobs.json` **必须**包含 `notify`；旧版 `context` 字段不再读取。

`assistant.defaults.notify` 为未指定 notify 的 Job 提供默认 IM 目标（如 master 私聊）。
...
zhin cron add "0 8 * * 1-5" "早报摘要" --label 工作日早报 --notify-channel im
