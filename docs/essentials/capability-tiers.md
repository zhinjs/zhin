...
>
> **平台档位 SSOT**：[`scripts/adapter-meta.mjs`](https://github.com/zhinjs/zhin/blob/main/scripts/adapter-meta.mjs)（[`docs/snippets/platform-tiers.md`](https://github.com/zhinjs/zhin/blob/main/docs/snippets/platform-tiers.md) 由 `pnpm sync:adapter-docs` 生成）。
...
**产品对标**：多通道 **生活/工作助手**（私聊/群聊、记忆、schedule、Home Assistant、通知）——不是 Cursor / Claude Code 类的 **写代码 Agent**，也不内置 **plan mode** 或以改仓库为主轴的 Harness。IM 是 Endpoint 的主场景之一，不是产品定义的全部。

## 产品边界（刻意不做 vs 计划中）

| 方向 | 态度 | 说明 |
|------|------|------|
| 多轮对话、工具、记忆、Compaction、会话树 | ✅ 核心（需 `@zhin.js/agent`） | 服务**对话与生活场景**；会话树是「聊天分支」，不是代码变更计划 |
...
| 档位 | 一句话 | 验证 |
|------|--------|------|
| **Stable（Core）** | 最少配置能跑：**Sandbox + 命令 + Console**（IM）；AI 另装，见 [full-bot](https://github.com/zhinjs/zhin/tree/main/examples/full-bot) | `pnpm check:stable` · [minimal-bot](https://github.com/zhinjs/zhin/tree/main/examples/minimal-bot) |
| **Platform Stable** | 主流 IM 适配器，已进 Stable smoke 的 Platform 批 | **当前无**（升档见 ADR 0015 D3）· [适配器索引](/adapters/) |
| **Advanced** | 编排增强：toolSearch、A2A Mesh、多 Endpoint 同进程 | [test-bot ACCEPTANCE](https://github.com/zhinjs/zhin/blob/main/examples/test-bot/ACCEPTANCE.md) |
...
| 内置运维命令 `/tools`、`/mcp` | Stable（需 agent 栈 + 启用） | [命令 — 内置 IM 运维](/essentials/commands#内置-im-运维命令adr-0010) |
...
**当前无。** 升档条件与检查清单见下方「维护者：升档检查清单」。现网常用 IM 适配器多数为 **Advanced** 或 **Experimental**，完整矩阵见 [平台适配器索引](/adapters/)（同源 SSOT）。
...
- MCP Client：filesystem 等（`mcpServers` 默认可为空；见 [MCP 集成](/advanced/mcp)）——需 agent 栈
- Agent：`spawn_task`、exec 策略、三层文件记忆、Bootstrap（SOUL/AGENTS/TOOLS）——需 agent 栈
...
flowchart TD
  ...
  q1 -->|要上 QQ/微信| q2{哪个平台}
  q2 --> advAdapters[适配器索引 Advanced/Experimental]
  q2 --> adv{要 toolSearch / A2A Mesh}
  adv -->|是| tb[test-bot / full-bot]
  adv -->|否| advAdapters
  min --> l2[L2 写命令插件]
  advAdapters --> prod[配置 bots + 实机验证]
```
...
| 纯命令 Endpoint | [命令系统](/essentials/commands) + [适配器索引](/adapters/) 任选平台包 |
| QQ / 微信系 | [qq](/adapters/qq)、[icqq](/adapters/icqq)、[wechat-mp](/adapters/wechat-mp)、[weixin-ilink](/adapters/weixin-ilink)、[napcat](/adapters/napcat)（档位见 SSOT） |
...
2. 更新 [`scripts/adapter-meta.mjs`](https://github.com/zhinjs/zhin/blob/main/scripts/adapter-meta.mjs) 的 `ADAPTER_META`（`tier: 'PlatformStable'`）；
3. 将测试路径加入 `scripts/run-stable-smoke.mjs` 的 Platform 批；
4. `pnpm sync:adapter-docs` + `pnpm check:platform-tiers-ssot` + `pnpm check:stable`；
