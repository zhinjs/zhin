...
**线上常见故障**：在中间件、命令 `.action()`、工具 `.execute()`、Cron、`.on()` 等**运行时回调**里调用 `getPlugin()`。跨 `await` 或部分平台回调时 AsyncLocalStorage 上下文可能丢失，抛出 `getPlugin() must be called within a plugin context`。
