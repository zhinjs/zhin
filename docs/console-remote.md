...
## 在线 Demo（demo.zhin.dev）

零安装体验官方托管实例（**独立站点**，非 console.zhin.dev 登录流）：

| 站点 | 用途 |
|------|------|
| **[demo.zhin.dev](https://demo.zhin.dev)** | Demo 静态壳：预连 API + Sandbox iframe + 首次访问 onboarding |
| **[demo-api.zhin.dev](https://demo-api.zhin.dev)** | 托管 Host（`examples/demo-bot` 配置） |

Demo 页 **无需** 填写 API Base / Token。首次访问展示 3 步引导（`hello` / `card` / `ai:` → 部署本机或接 QQ）；header 双 CTA 与 [deploy/zhin-demo/console/minimal](https://github.com/zhinjs/zhin/blob/main/deploy/zhin-demo/console/minimal/) 源码一致。

Demo 使用 **scoped Token**（`http.tokens[].scope: demo`），仅允许 Sandbox 聊天与白名单 RPC；详见 [ADR 0016 — Demo Host Token 作用域](/adr/0016-demo-host-token-scopes) 与 [deploy/zhin-demo](https://github.com/zhinjs/zhin/blob/main/deploy/zhin-demo/README.md)。

自托管 Demo 配置见 [examples/demo-bot](https://github.com/zhinjs/zhin/tree/main/examples/demo-bot)。
