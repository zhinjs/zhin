...
function signature(markdown) {
  const codeBlocks = matches(markdown, /^```[^\n]*\n[\s\S]*?^```[ \t]*$/gm);
...
const expected = manifest.pages.map(page => page.file).sort();
const actual = fs.readdirSync(zhDir).filter(file => file.endsWith('.md')).sort();
if (JSON.stringify(actual) !== JSON.stringify(expected)) {
  throw new Error(`Chinese Wiki page set differs: expected ${expected.length}, got ${actual.length}`);
}
...
for (const page of manifest.pages) {
  ...
  if (!chinese.includes(`translation_source_body_sha256: ${page.body_sha256}`)) {
