...
> 技术实现：[目标技术实现蓝图](./docs/architecture/target-implementation/README.md)。
...
setup/start 为 parent-first，ready 在 children ready 后向上确认；关闭时 Root 先阻止新任务并 drain，随后 children-first dispose，Root 基础 Resource 最后释放。详见 [ADR 0047](./docs/adr/0047-standalone-plugin-and-root-lifecycle-domain.md)。
...
子 Plugin instance key 占用父配置节点中的同名属性。若父 `schema.json.properties` 已声明同名字段，PluginLoader 必须报告 schema/child collision，不能静默覆盖。详见 [ADR 0045](./docs/adr/0045-hierarchical-plugin-config-schema.md)。
...
Command、Component、Skill、Tool、Agent、Page 和 Middleware 等能力由 `packages/*` 或 npm 中的 Feature provider 提供。Kernel 只认识稳定 `FeatureId` 与 owner-bound Slot，不内建能力枚举和目录表。Feature provider 拥有 definition helper、发现约定、验证和 generation projection。详见 [ADR 0048](./docs/adr/0048-plugin-monorepo-and-feature-provider-packages.md)。
...
`pages/` 是浏览器侧 Page 与 Layout Feature。普通文件产生 Page route：A 的 `pages/name.tsx` 对应 `/a/p-name`，A 的子 Plugin B 中 `pages/foo.tsx` 对应 `/a/b/p-foo`。保留文件 `$nav.tsx`、`$footer.tsx` 不产生 route，而是覆盖 owner Plugin 子树的 Console layout slot，并按最近祖先回退。左侧导航模型仍从 PageFeature snapshot 与 Plugin tree 派生，不由 `$nav.tsx` 自行维护。详见 [ADR 0046](./docs/adr/0046-convention-pages-and-plugin-navigation.md)。
...
- [ADR 0042：Capability Features 与按需 Capability Ingress](./docs/adr/0042-capability-features-and-on-demand-ingress.md)
- [ADR 0043：统一 Capability Root 与声明式文件接口](./docs/adr/0043-unify-capability-roots.md)
- [ADR 0044：TypeScript HMR Plugin Kernel](./docs/adr/0044-typescript-hmr-plugin-kernel.md)
- [ADR 0045：层级 Plugin 配置与 schema.json](./docs/adr/0045-hierarchical-plugin-config-schema.md)
- [ADR 0046：约定式 pages 与 Plugin 导航树](./docs/adr/0046-convention-pages-and-plugin-navigation.md)
- [ADR 0047：独立 Plugin 项目与 Root 生命周期域](./docs/adr/0047-standalone-plugin-and-root-lifecycle-domain.md)
