...
describe('Workroom shared Pack publisher authority gate', () => {
  it('never upgrades an HTTP principal to control-plane Root', async () => {
    ...
    expect(planningCoordinator).not.toMatch(
      /publishPack[\s\S]{0,900}authenticatedPrincipalId:\s*WORKROOM_CONTROL_PLANE_ROOT_PRINCIPAL/u,
    );
    expect(planningCoordinator).toContain('authenticatedPrincipalId: authenticatedPrincipal.principalId');
    expect(planningCoordinator).toContain('Control-plane Root Pack bootstrap is not exposed through Console HTTP');
