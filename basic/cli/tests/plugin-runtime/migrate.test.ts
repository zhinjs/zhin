...
import { LegacyCapabilityMigrator } from '../../src/plugin-runtime/migrate/index.js';
...
describe('legacy capability migration', () => {
  it('extracts static builders and inventories unsafe closures without editing legacy source', async () => {
    ...
    expect(plan.changes).toEqual([expect.objectContaining({
      ...
      source,
      target: join(root, 'commands/gh/pr/[title]/index.ts'),
    })]);
  ...
  it('rejects colliding command routes before writing any target', async () => {
    ...
    await expect(readFile(join(root, 'commands/status/index.ts'), 'utf8')).rejects.toThrow();
  });
...
`);
    ...
    expect(plan.changes.map(({ kind, identity, target }) => ({ kind, identity, target }))).toEqual([
      {
        ...
        target: join(root, 'middlewares/request-audit/index.ts'),
      },
      {
        kind: 'component',
        identity: 'status-card',
        target: join(root, 'components/status-card/index.ts'),
      },
    ]);
    ...
    await migrator.apply(plan);
    await expect(readFile(join(root, 'middlewares/request-audit/index.ts'), 'utf8'))
      .resolves.toContain('defineMiddleware');
    await expect(readFile(join(root, 'components/status-card/index.ts'), 'utf8'))
      .resolves.toContain('defineComponent');
    const component = await readFile(join(root, 'components/status-card/index.ts'), 'utf8');
...
async function temp(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'zhin-runtime-migrate-'));
  temporary.push(root);
  return root;
}
