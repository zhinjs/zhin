...
define(['./Kit', './parse'], function (K, parse) {
  ...
  function plotTree(tree, x, y) {
    ...
    var lineInX = results[0].lineInX,
      lineOutX = results[results.length - 1].lineOutX;
    return {
      items: items,
      width: width,
      height: height,
  ...
  function textRect(s, x, y, bgColor, textColor) {
    ...
    var tw = s.length * charSize.width,
  ...
  function textLabel(x, y, s, color) {
    // x is center x ,y is bottom y
  ...
  var plotNode = {
    ...
    repeat: function (node, x, y) {
      ...
      var offsetX = padding,
        offsetY = 0,
        r = padding,
        rectH = ret.y + ret.height - y,
        rectW = padding * 2 + ret.width;
      width = rectW;
      ...
      if (repeat.min === 0) {
        //draw a skip path
        var skipRectH = y - ret.y + padding,
          skipRectW = rectW + padding * 2;
        offsetX += padding;
    ...
    charset: function (node, x, y) {
      ...
      var charBgColor = 'LightSkyBlue',
        charTextColor = 'black',
        clsBgColor = 'Green',
        clsTextColor = 'white',
        rangeBgColor = 'teal',
        rangeTextColor = 'white',
    ...
    group: function (node, x, y) {
      ...
      if (node.num) {
        ...
        var rectW = sub.width + padding * 2,
          rectH = sub.height + padding * 2;
        var rect = {
          'type': 'rect',
          'x': x,
          'y': sub.y - padding,
    ...
    assert: function (node, x, y) {
      ...
      var conf,
        nat = node.assertionType,
        txt = nat.replace('Assert', '') + '!';
      ...
      var rectH = sub.height + padding * 2,
        rectW = sub.width + padding * 2;
      var rect = {
        'type': 'rect',
        'x': x,
        'y': sub.y - padding,
